...
export default function LoginPage() {
  ...
  const onSubmit = (data: LoginForm) => {
    ...
    // Simulate API call
    setTimeout(() => {
      setMsg(`Welcome back, ${data.email}! (This is a mock login)`);
      setIsLoading(false);
    }, 1000);
  };
        ...
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
          <div>
            ...
            <div className="relative">
              <input
                type="email"
                className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 bg-gray-50/50"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-green-600 to-teal-600 text-white font-semibold hover:from-green-700 hover:to-teal-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-[1.02] transition-all duration-200 shadow-lg"
          >
            {isLoading ? (
